...
def test_clean_data_rename_cols(raw_data):
    data = clean_reformat_data(raw_data)
    assert np.all(np.isin(["age_category","overall_experience","in_field_experience","education"],data.columns.values))
...
def test_mapper_age(raw_data):
    ...
    # Init Mapper
    transformer = Mapper(variables=config.model_config.AGE_VARS,
...
def test_mapper_overall_experience(raw_data):
    ...
    # Init Mapper
    transformer = Mapper(variables=config.model_config.EXPERIENCE_VARS,
