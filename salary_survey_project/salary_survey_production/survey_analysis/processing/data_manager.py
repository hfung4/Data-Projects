...
def clean_reformat_data(raw:pd.DataFrame, new_input_data:bool=False) ->pd.DataFrame:
    """
    clean_reformat_data: this function perform data cleaning and reformating-- steps that could be
    done outside of data pipeline (wouldn't lead to data leakage)

    Args:
        raw(pd.DataFrame): raw dataframe
        new_input(bool): if input data is a new/unknown dataset with no salary column, then True
    ...
    transformed.loc[:, 'timestamp'] = transformed.loc[:, 'timestamp'].astype('datetime64[ns]')
    ...
    transformed['overall_years_of_professional_experience'] = (
        transformed['overall_years_of_professional_experience']
        .str
        .replace(' - ', '-'))

    # cast overall_years_of_professional_experience as an ordered categorical variable
    parse_num_get_levels(transformed,
                         "overall_years_of_professional_experience")

    # years_of_experience_in_field
    transformed['years_of_experience_in_field'] = (transformed['years_of_experience_in_field']
                                                  .str
                                                  .replace(' - ', '-'))
    ...
    transformed["how_old_are_you"] = transformed["how_old_are_you"].replace(to_replace="under 18",
                                                                          value="17 or under")
    ...
    # rename variables
    transformed = transformed.rename(columns={"how_old_are_you": "age_category",
    ...
    if not new_input_data:
        transformed = remove_outlier(df_in=transformed,
                                    col="annual_salary",
